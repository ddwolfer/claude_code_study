...
console.log(`\nClaude Code in Action study coach — setup check`)
...
// 3) Gemini API key -------------------------------------------------------
head('GEMINI_API_KEY (gemini-video — 本課程為純文字,用不到,選用)')
{
  ...
  else warn('未設定 — 本課程是純文字 HTML,直接 Read 即可,gemini-video 用不到,可略過。\n       (只有要用 Gemini 看影片/PDF 的其它課才需要)')
}

// 4) Knowledge graph db (your captured knowledge) -------------------------
head('Knowledge graph (kg/claude-code.db)')
{
  const db = 'kg/claude-code.db'
...
head('Course material (Claude Code 實戰_課程講義/)')
{
  ...
  if (exists(lessonsDir)) {
    ...
    warn(`${lessonsDir}/ missing — run:  node scripts/extract-course.mjs  (從 course/ 萃取 21 課)。\n       座艙的課程目錄靠它列出;純查 KG 複習則不需要。`)
  }
}
