 ...
 *
 * The KG runs SQLite in WAL mode: new captures sit in kg/claude-code.db-wal
...
const DB = join(ROOT, 'kg', 'claude-code.db')
...
git('add', 'kg/claude-code.db')
const changed = git('diff', '--cached', '--quiet', '--', 'kg/claude-code.db').status === 1
if (!changed) {
...
const c = git('commit', 'kg/claude-code.db', '-m', msg)
