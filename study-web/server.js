...
// Bind address. Defaults to 127.0.0.1 (loopback only — the security model: no
// auth, localhost-only). Set STUDY_WEB_HOST=0.0.0.0 to expose on the LAN so
// other devices (tablet/phone/another PC) can reach it via the host's LAN IP.
const HOST = process.env.STUDY_WEB_HOST ?? '127.0.0.1'
const INDEX_HTML = join(__dirname, 'public', 'index.html')
const STATE_FILE = join(__dirname, 'state.json')
const PROJECT_ROOT = join(__dirname, '..')
const LESSONS_ROOT = join(PROJECT_ROOT, 'Claude Code 實戰_課程講義')
...
const httpServer = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/video/')) {
    let segs
    try { segs = url.pathname.slice('/video/'.length).split('/').map(decodeURIComponent) }
    catch { res.writeHead(400, { 'content-type': 'text/plain; charset=utf-8' }).end('bad video path'); return }
    if (segs.length !== 3 || segs.some(s => !s || s.includes('..') || /[\\/]/.test(s)) || !VIDEO_RE.test(segs[2])) {
      res.writeHead(400, { 'content-type': 'text/plain; charset=utf-8' }).end('bad video path')
      return
    }
    const filePath = join(LESSONS_ROOT, segs[0], segs[1], segs[2])
    let size
    try { size = statSync(filePath).size }
    catch { res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' }).end('video not found'); return }
    const type = /\.webm$/i.test(segs[2]) ? 'video/webm'
      : /\.mov$/i.test(segs[2]) ? 'video/quicktime' : 'video/mp4'
    ...
    if (range) {
      ...
      let start, end
      if (m && m[1] === '' && m[2] !== '') {     // suffix range "bytes=-N" → last N bytes
...
// Return the first non-internal IPv4 LAN address (e.g. 192.168.x.x), or null.
function lanAddress() {
...
// listen on HOST (default 127.0.0.1), fall forward on EADDRINUSE so a stale
// process can't take the whole MCP server down.
function listen(port, attemptsLeft = 10) {
  ...
  httpServer.listen(port, HOST, () => {
    ...
    if (HOST !== '127.0.0.1' && HOST !== 'localhost') {
      ...
      console.error(`[study-web] LAN access ENABLED (host=${HOST}). No auth — trusted networks only!`)
      if (lan) console.error(`[study-web] On other devices open: http://${lan}:${port}`)
    }
  })
}
listen(DEFAULT_PORT)
